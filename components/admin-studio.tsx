  ...
  return (
    <div className="admin-shell">
      <header className="admin-app-header">
        <div className="admin-app-brand"><span className="admin-app-mark">S</span><div><span className="eyebrow">SHEF / private kitchen</span><strong>Content control</strong></div></div>
        <div className="admin-app-actions"><a href="/" target="_blank" rel="noreferrer">View site <ArrowUpRight size={16} weight="bold" /></a><span className="admin-account">{userEmail}</span><button type="button" onClick={() => void signOut()}><SignOut size={16} /> Sign out</button></div>
      </header>

      <div className="admin-app-body">
        <aside className="admin-sidebar" aria-label="CMS sections">
          <div className="admin-side-label">Manage</div>
          <nav>
          ...
          <div className="admin-side-foot"><span className="admin-status-dot" /> Supabase connected</div>
        </aside>
...
function LoadingState() {
  return <div className="admin-loading"><SpinnerGap size={24} className="admin-spin" /><span>Loading your content…</span></div>;
}

function ErrorState({ onRetry }: { onRetry: () => void }) {
  return <section className="admin-empty admin-error-state"><X size={26} /><h2>Could not open the kitchen.</h2><p>Check the Supabase connection, then try again.</p><button className="admin-button admin-button-dark" type="button" onClick={onRetry}><ArrowClockwise size={16} /> Try again</button></section>;
}
...
function Overview({ content, counts, onCreate, onNavigate }: { content: CmsContent; counts: { gallery: number; galleryLive: number; events: number; eventsLive: number; mixes: number; mixesLive: number }; onCreate: (resource: Resource) => void; onNavigate: (section: Section) => void }) {
  return <>
    <div className="admin-page-intro"><div><span className="eyebrow">Dashboard / SHEF</span><h1>Make it <em>live.</em></h1><p>Manage the visual archive, upcoming events, and recorded sets that appear across the site.</p></div><button className="admin-button admin-button-dark" type="button" onClick={() => onCreate("event")}><Plus size={18} weight="bold" /> Add event</button></div>
    ...
    <section className="admin-overview-grid">
      <div className="admin-card admin-quick-card"><div className="admin-card-heading"><div><span className="eyebrow">Quick publish</span><h2>What are you adding?</h2></div></div><div className="admin-quick-actions"><button type="button" onClick={() => onCreate("gallery")}><Images size={22} /><span><strong>Gallery image</strong><small>Upload a new visual to the archive</small></span><Plus size={18} /></button><button type="button" onClick={() => onCreate("event")}><CalendarBlank size={22} /><span><strong>Event</strong><small>Add a show, tour stop, or radio feature</small></span><Plus size={18} /></button><button type="button" onClick={() => onCreate("release")}><MusicNote size={22} /><span><strong>Mix or release</strong><small>Add a set with its SoundCloud link</small></span><Plus size={18} /></button></div></div>
      <div className="admin-card admin-recent-card"><div className="admin-card-heading"><div><span className="eyebrow">Latest events</span><h2>On the calendar</h2></div><button className="admin-text-button" type="button" onClick={() => onNavigate("events")}>View all <ArrowUpRight size={15} /></button></div>{content.appearances.length ? <div className="admin-mini-list">{content.appearances.slice(0, 3).map((item) => <div className="admin-mini-row" key={item.id}><span>{displayDate(item.year)}</span><strong>{item.city}</strong><small>{item.venue}</small><StatusBadge published={item.published} /></div>)}</div> : <p className="admin-muted">No events yet. Add the first one from the Events section.</p>}</div>
    </section>
  </>;
}
...
function EventForm({ draft, setDraft, editingId, busy, onClose, onSave }: { draft: EventDraft; setDraft: React.Dispatch<React.SetStateAction<EventDraft>>; editingId: string | null; busy: Resource | "upload" | null; onClose: () => void; onSave: (event: FormEvent<HTMLFormElement>) => void }) {
  return <form className="admin-editor-card" onSubmit={onSave}><div className="admin-form-heading"><div><span className="eyebrow">{editingId ? "Edit event" : "New event"}</span><h2>{editingId ? "Shape the details." : "Add a date to the orbit."}</h2></div><button className="admin-close-button" type="button" aria-label="Close form" onClick={onClose}><X size={20} /></button></div><div className="admin-field-grid"><label className="admin-field"><span>Date or year</span><input value={draft.year} onChange={(event) => setDraft((current) => ({ ...current, year: event.target.value }))} placeholder="2026 or 2026-11-14" required /></label><label className="admin-field"><span>City / location</span><input value={draft.city} onChange={(event) => setDraft((current) => ({ ...current, city: event.target.value }))} placeholder="Naarm / Melbourne" required /></label><label className="admin-field"><span>Venue or event name</span><input value={draft.venue} onChange={(event) => setDraft((current) => ({ ...current, venue: event.target.value }))} placeholder="STEMS Official" /></label><label className="admin-field"><span>Event link</span><input type="url" value={draft.href} onChange={(event) => setDraft((current) => ({ ...current, href: event.target.value }))} placeholder="https://instagram.com/…" /></label><label className="admin-field admin-field-wide"><span>Details / set note</span><textarea value={draft.note} onChange={(event) => setDraft((current) => ({ ...current, note: event.target.value }))} placeholder="Opening set · tickets · lineup details" rows={3} /></label></div><PublishToggle published={draft.published} onChange={(published) => setDraft((current) => ({ ...current, published }))} /><div className="admin-form-actions"><button className="admin-button admin-button-dark" type="submit" disabled={busy === "event"}>{busy === "event" ? <SpinnerGap size={17} className="admin-spin" /> : <Check size={17} />} {editingId ? "Save changes" : "Publish event"}</button><button className="admin-button admin-button-light" type="button" onClick={onClose}>Cancel</button></div></form>;
}
